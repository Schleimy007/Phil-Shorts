...
// 1. DYNAMISCHES CSS FÜR KARTEN
if (!document.getElementById('premium-live-styles')) {
    const style = document.createElement('style');
    style.id = 'premium-live-styles';
    style.innerHTML = `
        .ultra-live-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 15px; }
        .premium-live-card { background: #111; border-radius: 12px; overflow: hidden; cursor: pointer; position: relative; border: 1px solid #333; }
        .card-thumbnail { position: relative; height: 200px; width: 100%; overflow: hidden; display: flex; align-items: center; justify-content: center; }
        .thumb-bg { position: absolute; width: 100%; height: 100%; background-size: cover; filter: blur(15px); opacity: 0.5; }
        .card-thumbnail img { position: relative; z-index: 1; height: 100%; width: 100%; object-fit: cover; }
        .live-badge-glow { position: absolute; top: 10px; left: 10px; background: #ff0050; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: bold; z-index: 2; animation: pulse 2s infinite; }
        .viewer-count { position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.6); color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; z-index: 2; }
        .card-info { padding: 10px; background: #1a1a1a; display: flex; gap: 10px; align-items: center; }
        .card-info .avatar { width: 30px; height: 30px; border-radius: 50%; object-fit: cover; }
        .card-info .text-info { flex: 1; min-width: 0; }
        .card-info h4 { margin: 0; font-size: 13px; color: white; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .card-info span { font-size: 11px; color: #888; }
    `;
    document.head.appendChild(style);
}

// 2. LIVE LISTE RENDERN
window.LiveManager = {
    unsubscribe: null,
    init: function() {
            ...
            if (snapshot.empty) {
                grid.innerHTML = `
                    <div style="text-align: center; margin-top: 50px;">
                        <i class="fas fa-video-slash" style="font-size: 40px; color: #444; margin-bottom: 15px;"></i>
                        <h3 style="color: white;">Niemand ist live</h3>
                    </div>
                `;
                return;
            }
...
function initLiveRoomListeners(streamId) {
    ...
    const chatUnsub = onSnapshot(query(collection(db, `live_streams/${streamId}/chat`), orderBy("timestamp", "desc"), limit(30)), snap => {
        ...
        snap.forEach(d => {
            const m = d.data();
            const hostBadge = m.uid === streamId ? '<span style="background:#ff0050; color:white; font-size:10px; padding:2px 4px; border-radius:3px; margin-right:5px;">HOST</span>' : '';
            chatBox.innerHTML += `<div class="lr-chat-msg">
                ${hostBadge}<strong onclick="openProfile('${m.uid}')">${m.name}:</strong> <span>${m.text}</span>
            </div>`;
        });
    });
