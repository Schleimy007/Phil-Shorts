...
let dragState = { activeId: null, isVideo: false, activeResizeId: null, activeResizePos: null, startX: 0, startY: 0, initialObjX: 0, initialObjY: 0, startSize: 0, startDist: 0, gridVisible: false };
...
window.sendDesktopNotification = function(title, body, type) {
    ...
    try { const notif = new Notification(title, { body: body }); notif.onclick = function() { window.focus(); this.close(); }; } catch (e) { console.error(e); }
};

function updateNotifUI() {
    const masterToggle = document.getElementById('notif-master'); const subSettings = document.getElementById('notif-sub-settings'); if (!masterToggle) return;
    masterToggle.checked = notifSettings.master; document.getElementById('notif-comments').checked = notifSettings.comments; document.getElementById('notif-likes').checked = notifSettings.likes; document.getElementById('notif-dms').checked = notifSettings.follows; document.getElementById('notif-follows').checked = notifSettings.follows;
...
document.getElementById('notif-master').addEventListener('change', async(e) => {
    if (e.target.checked) {
        ...
        if (Notification.permission === "denied") { showCustomAlert("Blockiert!", "Du hast Benachrichtigungen blockiert."); e.target.checked = false; notifSettings.master = false; updateNotifUI(); return; }
...
let currentFeedMode = 'foryou'; let isInitialLoad = true; let sortedFeed = []; const viewedVideos = new Set(); window.globalVolume = 1; window.globalMuted = false;

window.switchView = function(viewId) {
    document.querySelectorAll('.view').forEach(v => v.classList.remove('active')); document.getElementById('view-' + viewId).classList.add('active'); document.querySelectorAll('.nav__item').forEach(n => n.classList.remove('active'));
    if (viewId === 'feed') document.querySelectorAll('.nav__item')[0].classList.add('active'); if (viewId === 'search') document.querySelectorAll('.nav__item')[1].classList.add('active'); if (viewId === 'inbox' || viewId === 'dm' || viewId === 'ticket') document.querySelectorAll('.nav__item')[3].classList.add('active'); if (viewId === 'profile' && currentUser && document.getElementById('profile-name').innerText.includes(currentUser.displayName)) document.querySelectorAll('.nav__item')[4].classList.add('active');
    if (viewId !== 'feed') document.querySelectorAll('.video__player').forEach(v => { v.pause(); v.currentTime = 0; });
    const audioPlayer = document.getElementById('profile-audio-player'); if (viewId !== 'profile' && audioPlayer) { audioPlayer.pause(); }
};

window.jumpToVideo = function(videoId) { switchView('feed'); setTimeout(() => { const targetVid = document.querySelector(`.video[data-id="${videoId}"]`); if (targetVid) { targetVid.scrollIntoView({ behavior: 'smooth', block: 'center' }); document.querySelectorAll('.video__player').forEach(v => { v.pause(); v.currentTime = 0; }); const player = targetVid.querySelector('.video__player'); if (player) { player.muted = window.globalMuted; player.play().catch(() => {}); } } }, 250); };
...
window.formatText = function(text) { if (!text) return ""; let safeText = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); safeText = safeText.replace(/#([a-zA-Z0-9_äöüÄÖÜß]+)/g, '<span class="hashtag" onclick="openHashtag(\'$1\', event)">#$1</span>'); safeText = safeText.replace(/@([a-zA-Z0-9_]+)/g, '<span class="mention" onclick="openProfileByUsername(\'$1\', event)">@$1</span>'); return safeText; };

window.openHashtag = function(tag, event) { if (event) event.stopPropagation(); switchView('hashtag'); document.getElementById('hashtag-title').innerText = '#' + tag; const grid = document.getElementById('hashtag-grid'); grid.innerHTML = ''; let blocked = (currentUser && currentUser.blockedUsers) ? currentUser.blockedUsers : []; const matchedVideos = allVideosData.filter(v => (v.description || "").toLowerCase().includes('#' + tag.toLowerCase()) && !blocked.includes(v.authorUid)); if (matchedVideos.length === 0) { grid.innerHTML = '<div style="grid-column: span 3; text-align: center; margin-top: 50px; color: #555;">Keine Videos gefunden</div>'; return; } matchedVideos.forEach(v => { const previewSrc = v.mediaType === 'images' && v.urls ? v.urls[0] : `${v.url}#t=0.5`; const mediaTag = v.mediaType === 'images' ? `<img src="${previewSrc}" style="width:100%; height:100%; object-fit:cover;">` : `<video src="${previewSrc}" muted playsinline style="width:100%; height:100%; object-fit:cover;"></video>`; const icon = v.mediaType === 'images' ? 'fa-images' : 'fa-play'; grid.innerHTML += `<div class="grid-item" onclick="jumpToVideo('${v.id}')">${mediaTag}<div class="grid-views"><i class="fas ${icon}"></i> ${v.likedBy ? v.likedBy.length : 0}</div></div>`; }); };

window.openProfileByUsername = function(username, event) { if (event) event.stopPropagation(); const user = allKnownUsers.find(u => { const cleanUname = u.username || (u.displayName ? u.displayName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase() : 'user'); return cleanUname === username.toLowerCase(); }); if (user) openProfile(user.uid); else showToast("Nutzer @" + username + " nicht gefunden!"); };

let activeMentionInput = null; let mentionStartIndex = -1;
document.addEventListener('input', (e) => { if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') { const val = e.target.value; const cursorPos = e.target.selectionStart; if (cursorPos === undefined) return; const textBeforeCursor = val.substring(0, cursorPos); const match = textBeforeCursor.match(/@([a-zA-Z0-9_]*)$/); if (match) { activeMentionInput = e.target; mentionStartIndex = cursorPos - match[0].length; showMentionSuggestions(e.target, match[1].toLowerCase()); } else hideMentionSuggestions(); } });

window.showMentionSuggestions = function(inputEl, query) { let blocked = (currentUser && currentUser.blockedUsers) ? currentUser.blockedUsers : []; const matchedUsers = allKnownUsers.filter(u => { if(blocked.includes(u.uid)) return false; const uname = (u.username || "").toLowerCase(); const dname = (u.displayName || "").toLowerCase(); return uname.includes(query) || dname.includes(query); }); const box = document.getElementById('mention-suggestions'); if (!box) return; if (matchedUsers.length === 0) { box.style.display = 'none'; return; } box.innerHTML = matchedUsers.map(u => { const safeUsername = u.username || (u.displayName ? u.displayName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase() : 'user'); const safePic = u.photoURL || 'https://api.dicebear.com/7.x/avataaars/svg?seed=fallback'; return `<div class="mention-item" onclick="selectMention('${safeUsername}')"><img src="${safePic}"><span>@${safeUsername}</span></div>`; }).join(''); const rect = inputEl.getBoundingClientRect(); const spaceBelow = window.innerHeight - rect.bottom; box.style.left = rect.left + 'px'; if (spaceBelow > 200) { box.style.top = (rect.bottom + 5) + 'px'; box.style.bottom = 'auto'; } else { box.style.top = 'auto'; box.style.bottom = (window.innerHeight - rect.top + 5) + 'px'; } box.style.display = 'block'; };
...
function checkPhilPlusStatus(requiredTier = 1) { if (!currentUser) return false; return currentUser.philPlusUntil && currentUser.philPlusUntil > Date.now() && (currentUser.philPlusTier || 1) >= requiredTier; }
...
function getInlineBorderStyle(activeBorder, customBorder) { if (!activeBorder || activeBorder === 'none') return ''; if (activeBorder === 'custom' && customBorder) { if (customBorder.grad) { return `border: 3px solid transparent; background: linear-gradient(#000, #000) padding-box, linear-gradient(45deg, ${customBorder.c1}, ${customBorder.c2}) border-box; padding: 2px;`; } else { return `border: 3px solid ${customBorder.c1}; box-shadow: 0 0 10px ${customBorder.c1}, inset 0 0 5px ${customBorder.c1}; padding: 2px;`; } } return ''; }
function getBorderClass(activeBorder) { if (!activeBorder || activeBorder === 'none' || activeBorder === 'custom') return ''; return `border-${activeBorder}`; }
...
function initLiveUser() {
    ...
    userUnsubscribe = onSnapshot(doc(db, "users", currentUser.uid), (docSnap) => {
        if (docSnap.exists()) {
            ...
            if (!currentUser.followers) currentUser.followers = []; if (!currentUser.following) currentUser.following = []; if (!currentUser.savedVideos) currentUser.savedVideos = []; if (!currentUser.blockedUsers) currentUser.blockedUsers = []; if (!currentUser.closeFriends) currentUser.closeFriends = []; if (!currentUser.socialLinks) currentUser.socialLinks = {ig: '', yt: '', tw: '', tt: ''}; if (!currentUser.username) currentUser.username = currentUser.displayName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase(); if (!currentUser.decorations) currentUser.decorations = []; if (!currentUser.appTheme) currentUser.appTheme = 'default'; if (!currentUser.appIcon) currentUser.appIcon = 'default'; if (!currentUser.philPlusTier) currentUser.philPlusTier = 0; if (!currentUser.customBorder) currentUser.customBorder = { c1: '#ff0050', c2: '#00f2fe', grad: true };

            const today = new Date().toDateString();
            if (currentUser.lastLogin !== today) {
                let yesterday = new Date(Date.now() - 86400000).toDateString();
                let newStreak = 1;
                if(currentUser.lastLogin === yesterday) { newStreak = (currentUser.streak || 0) + 1; }
                currentUser.streak = newStreak;
                
                let bonus = 100; if (checkPhilPlusStatus(3)) bonus = 500; else if (checkPhilPlusStatus(2)) bonus = 200;
                currentUser.coins += bonus; currentUser.lastLogin = today;
                updateDoc(doc(db, "users", currentUser.uid), { coins: currentUser.coins, lastLogin: today, lastActive: Date.now(), streak: newStreak });
                showToast(`Tag ${newStreak} Streak🔥! +${bonus} Coins erhalten!`);
            }
            ...
            if (checkPhilPlusStatus(3)) { document.getElementById('tier3-settings-area').style.display = 'block'; document.getElementById('account-switcher-area').style.display = 'block'; document.getElementById('up-story-link').style.display = 'block'; document.getElementById('btn-live-stream').style.display = 'flex'; } else { document.getElementById('tier3-settings-area').style.display = 'none'; document.getElementById('account-switcher-area').style.display = 'none'; document.getElementById('up-story-link').style.display = 'none'; document.getElementById('btn-live-stream').style.display = 'none'; }
            ...
            const coinEl = document.getElementById('my-coins'); if (coinEl) coinEl.innerText = currentUser.coins; const viewsEl = document.getElementById('my-views'); if (viewsEl) viewsEl.innerText = currentUser.profileViews || 0; const actionBtn = document.getElementById('profile-action-btn');

            if (actionBtn && actionBtn.dataset.uid === currentUser.uid) {
                document.getElementById('stat-followers').innerText = currentUser.followers.length; document.getElementById('stat-following').innerText = currentUser.following.length;
...
function initSearchUsers() {
    onSnapshot(collection(db, "users"), (snapshot) => {
        ...
        allKnownUsers.forEach(u => {
            ...
            document.querySelectorAll(`.live-username-${u.uid}`).forEach(el => el.innerText = '@' + cleanUsername); document.querySelectorAll(`.live-pic-${u.uid}`).forEach(el => { el.src = u.photoURL; applyBorderStyles(el, u.activeBorder, u.customBorder); });
        });
    });
}
...
window.addEventListener('googleLoginSuccess', async(event) => {
    try {
        ...
        if (!userSnap.exists()) {
            let finalUser = baseUser; let nameQuery = query(collection(db, "users"), where("username", "==", finalUser)); let nameSnap = await getDocs(nameQuery); while (!nameSnap.empty) { finalUser = baseUser + Math.floor(1000 + Math.random() * 9000); nameQuery = query(collection(db, "users"), where("username", "==", finalUser)); nameSnap = await getDocs(nameQuery); }
            const newUser = { uid: uid, displayName: rawDisplayName, username: finalUser, email: email, photoURL: pic, bio: "Neu in der Community! 👋", following: [], followers: [], savedVideos: [], blockedUsers: [], closeFriends: [], socialLinks: {ig: '', yt: '', tw: '', tt: ''}, verified: false, coins: 1000, profileViews: 0, isAdmin: false, banned: false, decorations: [], activeBorder: "", stories: [], appTheme: 'default', philPlusTier: 0, lastLogin: new Date().toDateString(), lastActive: Date.now(), streak: 1, customBorder: { c1: '#ff0050', c2: '#00f2fe', grad: true } };
            ...
            if (!currentUser.following) currentUser.following = []; if (!currentUser.savedVideos) currentUser.savedVideos = []; if (!currentUser.blockedUsers) currentUser.blockedUsers = []; if (!currentUser.closeFriends) currentUser.closeFriends = []; if (!currentUser.socialLinks) currentUser.socialLinks = {ig: '', yt: '', tw: '', tt: ''}; if (!currentUser.decorations) currentUser.decorations = []; if (!currentUser.username) currentUser.username = currentUser.displayName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase(); if (currentUser.coins === undefined) await updateDoc(userRef, { coins: 1000, profileViews: 0, followers: [] }); if (!currentUser.customBorder) await updateDoc(userRef, { customBorder: { c1: '#ff0050', c2: '#00f2fe', grad: true } });
        }
        localStorage.setItem('phil_session', JSON.stringify(currentUser)); document.getElementById('login-screen').classList.remove('show'); initLiveDatabase(); initLiveUser(); initInbox(); initInboxChats(); initSearchUsers();
    } catch (error) { showCustomAlert("Login Fehler", "Datenbank-Fehler beim Login."); }
});

window.onload = async function() {
    if (!currentUser) { document.getElementById('login-screen').classList.add('show'); } else { document.getElementById('login-screen').classList.remove('show'); if (!currentUser.username) currentUser.username = currentUser.displayName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase(); if (!currentUser.savedVideos) currentUser.savedVideos = []; if (!currentUser.blockedUsers) currentUser.blockedUsers = []; if (!currentUser.closeFriends) currentUser.closeFriends = []; if (!currentUser.socialLinks) currentUser.socialLinks = {ig: '', yt: '', tw: '', tt: ''}; initLiveDatabase(); initLiveUser(); initInbox(); initInboxChats(); initSearchUsers(); }
...
document.getElementById('btn-live-stream').addEventListener('click', () => { showCustomAlert("Live Streaming (Beta)", "Die Live-Streaming Funktion befindet sich aktuell in der Entwicklung. Als Plus+++ User erhältst du als Erstes Zugriff!"); });

async function addNotification(targetUid, type, text, videoId = null) { if (!currentUser || targetUid === currentUser.uid) return; let targetUser = allKnownUsers.find(u => u.uid === targetUid); if(targetUser && targetUser.blockedUsers && targetUser.blockedUsers.includes(currentUser.uid)) return; await addDoc(collection(db, "users", targetUid, "notifications"), { fromUid: currentUser.uid, fromName: currentUser.displayName, fromUsername: currentUser.username, fromPic: currentUser.photoURL, type: type, text: text, videoId: videoId, timestamp: Date.now() }); }

function applyAlgorithm(videos, mode) {
    let filteredVids = videos;
    if (currentUser) {
        filteredVids = videos.filter(v => {
            if (v.closeFriendsOnly && v.authorUid !== currentUser.uid) {
                const authorData = allKnownUsers.find(u => u.uid === v.authorUid);
                if (!authorData || !authorData.closeFriends || !authorData.closeFriends.includes(currentUser.username)) return false;
            }
            return true;
        });
    } else {
        filteredVids = videos.filter(v => !v.closeFriendsOnly);
    }

    if (mode === 'following') { let followedVids = filteredVids.filter(v => currentUser && currentUser.following && currentUser.following.includes(v.authorUid)); return followedVids.sort(() => Math.random() - 0.5); } else {
        let scoredVids = filteredVids.map(v => {
            let likes = v.likedBy ? v.likedBy.length : 0; let comments = v.comments ? v.comments.length : 0; let gifts = v.gifts || 0; let engagementScore = (likes * 5) + (comments * 10) + (gifts * 20); let baseViralPower = Math.log(engagementScore + 1) * 30; let authorData = allKnownUsers.find(u => u.uid === v.authorUid);
...
function initLiveDatabase() {
    ...
    onSnapshot(collection(db, "videos"), (snapshot) => {
        ...
        if (isInitialLoad) { renderFeed(true); isInitialLoad = false; const urlParams = new URLSearchParams(window.location.search); const sharedVideoId = urlParams.get('video'); if (sharedVideoId) { window.history.replaceState({}, document.title, window.location.pathname); setTimeout(() => jumpToVideo(sharedVideoId), 800); } } else {
            snapshot.docChanges().forEach((change) => {
                const vData = { id: change.doc.id, ...change.doc.data() }; if(blocked.includes(vData.authorUid)) return; 
                if (change.type === "added") { if (!document.querySelector(`.video[data-id="${vData.id}"]`)) { const newVidEl = createVideoElement(vData); if (currentFeedMode === 'foryou' || (currentFeedMode === 'following' && currentUser.following.includes(vData.authorUid))) { const container = document.getElementById('video-container'); const loader = container.querySelector('.feed-end-loader'); if (loader) container.insertBefore(newVidEl, loader); else container.appendChild(newVidEl); const emptyState = container.querySelector('.empty-state'); if (emptyState) emptyState.remove(); window.updateGlobalVolumeUI(); } } }
...
function renderFeed(reset = false) {
    ...
    if (reset) {
        ...
        let count = 0; sortedFeed.forEach(video => { container.appendChild(createVideoElement(video)); count++; if (!checkPhilPlusStatus(2) && count % 5 === 0) { container.appendChild(createAdElement()); } }); appendLoader(container, true);
    }
}

function appendLoader(container, isEnd) { const loader = document.createElement('div'); loader.className = 'feed-end-loader'; if (isEnd) { loader.innerHTML = '<i class="fas fa-check-circle"></i><span>Du bist auf dem neuesten Stand</span>'; loader.classList.add('no-more'); } else { loader.innerHTML = '<i class="fas fa-spinner fa-spin"></i><span>Prüfe Algorithmus...</span>'; } container.appendChild(loader); }

window.updateGlobalVolumeUI = function() {
    document.querySelectorAll('.video-inner').forEach(container => {
        const v = container.querySelector('.video__player'); const muteBtn = container.querySelector('.mute-btn'); const volumeSlider = container.querySelector('.volume-slider'); const bgAudio = container.querySelector('.bg-audio-player');
        if (!v || !muteBtn || !volumeSlider) return;
        let origVol = v.dataset.origVol ? parseFloat(v.dataset.origVol) : 1;
        let bgVol = bgAudio && bgAudio.dataset.bgVol ? parseFloat(bgAudio.dataset.bgVol) : 0.5;

        v.volume = window.globalVolume * origVol;
        if(bgAudio) bgAudio.volume = window.globalVolume * bgVol;

        v.muted = window.globalMuted;
        if(bgAudio) bgAudio.muted = window.globalMuted;

        if (window.globalMuted || window.globalVolume == 0) { muteBtn.innerHTML = '<i class="fas fa-volume-mute"></i>'; volumeSlider.value = 0; volumeSlider.style.background = `linear-gradient(to right, #fff 0%, rgba(255, 255, 255, 0.3) 0%)`; } else { if (window.globalVolume < 0.5) muteBtn.innerHTML = '<i class="fas fa-volume-down"></i>'; else muteBtn.innerHTML = '<i class="fas fa-volume-up"></i>'; volumeSlider.value = window.globalVolume; volumeSlider.style.background = `linear-gradient(to right, #fff ${window.globalVolume * 100}%, rgba(255, 255, 255, 0.3) ${window.globalVolume * 100}%)`; }
    });
};
...
function createVideoElement(video) {
    ...
    const authorData = getUserData(video.authorUid, video.authorName, video.authorUsername || video.authorName, video.authorPic, video.authorVerified); const verifiedBadge = getVerifiedBadge(authorData.verified);
    let tier3Badge = authorData.philPlusUntil > Date.now() && authorData.philPlusTier === 3 ? ' <i class="fas fa-gem" style="color: #00f2fe; font-size: 12px;" title="Plus+++ Legende"></i>' : ""; let nameClass = (authorData.philPlusUntil > Date.now() && authorData.philPlusTier >= 1) ? "name-phil-plus" : "";
    ...
    // Duett Button
    const duetBtn = `<div class="videoSidebar__button" onclick="openDuet('${video.id}')" style="margin-top:15px;"><i class="fas fa-record-vinyl" style="color: #fff;"></i><p>Duett</p></div>`;

    const mutedAttr = window.globalMuted ? 'muted' : ''; let mediaHTML = ''; let muteUIHtml = '';
    
    // Firebase Render Logik (Overlays & Filters)
    let videoFilter = video.filter ? video.filter : 'none';
    let cssFilter = 'none';
    if(videoFilter === 'grayscale') cssFilter = 'grayscale(100%)';
    if(videoFilter === 'sepia') cssFilter = 'sepia(100%)';
    if(videoFilter === 'blur') cssFilter = 'blur(4px)';
    if(videoFilter === 'contrast') cssFilter = 'contrast(200%)';
    if(videoFilter === 'vintage') cssFilter = 'sepia(50%) contrast(150%)';

    let overlayHtml = '';
    if(video.overlays && video.overlays.length > 0) {
        overlayHtml = video.overlays.map(obj => {
            let style = `position:absolute; left:${obj.x}px; top:${obj.y}px; transform:translate(-50%, -50%) rotate(${obj.rotation}deg); z-index:5; pointer-events:none;`;
            if(obj.type === 'text') {
                return `<div style="${style} font-size:${obj.size}px; color:${obj.color}; font-family:${obj.font}; font-weight:bold; text-shadow:1px 1px 3px black; text-align:center;">${obj.text}</div>`;
            } else if (obj.type === 'sticker') {
                return `<img src="${obj.src}" style="${style} width:${obj.size}px; height:auto;">`;
            } else if (obj.type === 'blur') {
                return `<div style="${style} width:${obj.size}px; height:${obj.size}px; backdrop-filter:blur(15px); -webkit-backdrop-filter:blur(15px); border-radius:10px;"></div>`;
            }
            return '';
        }).join('');
    }
    ...
    let bgAudioHtml = '';
    if(video.bgAudio) {
        bgAudioHtml = `<audio class="bg-audio-player" src="${video.bgAudio}" data-bg-vol="${video.bgAudioVol || 0.5}" preload="auto" loop></audio>`;
    }
    ...
    if (video.mediaType === 'images' && video.urls && video.urls.length > 0) {
        ...
        mediaHTML = `<video class="video__player" data-vid="${video.id}" data-orig-vol="${video.origVol || 1}" preload="auto" loop playsinline ${mutedAttr} src="${video.url}" style="filter: ${cssFilter};"></video>${bgAudioHtml}<div class="video-overlay-container" style="position:absolute; top:0; left:0; width:100%; height:100%; pointer-events:none; overflow:hidden;">${overlayHtml}</div><div class="play-indicator"><i class="fas fa-play"></i></div><div class="player-progress-bar"><div class="player-progress-filled"></div></div>`;
    ...
    let closeFriendsBadge = video.closeFriendsOnly ? `<span style="background:#39ff14; color:black; font-size:10px; padding:2px 5px; border-radius:5px; font-weight:bold; margin-bottom:5px; display:inline-block;"><i class="fas fa-star"></i> Enge Freunde</span><br>` : '';
    ...
    div.innerHTML = `<div class="video-inner is-paused"><div class="video-wrapper">${mediaHTML}${muteUIHtml}<div class="like-animation"><i class="fas fa-heart"></i></div><div class="gift-animation" id="gift-anim-${video.id}"></div></div><div class="video__footer">${closeFriendsBadge}<h3 class="creator-name" onclick="openProfile('${video.authorUid}')"><span class="live-name-${video.authorUid} ${nameClass}">${authorData.displayName}${verifiedBadge}${tier3Badge}</span></h3><p class="live-username-${video.authorUid}" style="color:#aaa; font-size:13px; margin-bottom:5px; cursor:pointer;" onclick="openProfile('${video.authorUid}')">@${authorData.username}</p><h4 class="video-title" onclick="openVideoDetails('${video.id}')">${video.title || 'Ohne Titel'}</h4><p class="video-desc-preview" onclick="openVideoDetails('${video.id}')">${previewHtml}</p></div><div class="video__sidebar"><div class="sidebar__profile" onclick="openProfile('${video.authorUid}')"><img src="${authorData.pic}" class="live-pic-${video.authorUid} ${bClass}" style="${inlineStyle}" alt="Profil">${plusButton}</div><div class="videoSidebar__button like-btn ${hasLiked}" data-id="${video.id}"><i class="fas fa-heart"></i><p class="like-count">${realLikes}</p></div><div class="videoSidebar__button comment-btn" data-id="${video.id}"><i class="fas fa-comment-dots"></i><p class="comment-count-txt">${commentCount}</p></div><div class="videoSidebar__button bookmark-btn ${hasSaved}" data-id="${video.id}" onclick="toggleSaveVideo('${video.id}', this)"><i class="fas fa-bookmark"></i><p>Speichern</p></div><div class="videoSidebar__button gift-btn" data-id="${video.id}"><i class="fas fa-gift" style="color: #ffd700;"></i><p class="gift-count">${video.gifts || 0}</p></div><div class="videoSidebar__button share-btn" data-id="${video.id}"><i class="fas fa-share"></i><p>Teilen</p></div>${duetBtn}${downloadVideoBtn}${analyticsBtn}${editVideoBtn}${deleteVideoBtn}</div></div>`;
    attachInteractionsToVideo(div); return div;
}

window.toggleSaveVideo = async function(videoId, btnEl) { if (!currentUser) return showCustomAlert("Fehler", "Bitte logge dich ein, um Videos zu speichern."); const isSaved = currentUser.savedVideos.includes(videoId); try { if(isSaved) { currentUser.savedVideos = currentUser.savedVideos.filter(id => id !== videoId); btnEl.classList.remove('saved'); await updateDoc(doc(db, "users", currentUser.uid), { savedVideos: arrayRemove(videoId) }); showToast("Aus Favoriten entfernt."); } else { currentUser.savedVideos.push(videoId); btnEl.classList.add('saved'); await updateDoc(doc(db, "users", currentUser.uid), { savedVideos: arrayUnion(videoId) }); showToast("Zu Favoriten hinzugefügt 📌"); } localStorage.setItem('phil_session', JSON.stringify(currentUser)); if(document.getElementById('view-profile').classList.contains('active') && window.currentProfileTab === 'saved') { renderProfileGrid(currentUser.uid); } } catch(e) { showCustomAlert("Fehler", "Konnte nicht gespeichert werden."); } };

window.openAnalytics = function(id) { const video = allVideosData.find(v => v.id === id); if(!video) return; document.getElementById('analytics-title').innerText = video.title || 'Analytics'; document.getElementById('analytics-views').innerText = video.views || 0; document.getElementById('analytics-likes').innerText = video.likedBy ? video.likedBy.length : 0; document.getElementById('analytics-gifts').innerText = video.gifts || 0; let views = video.views || 1; let eng = ((video.likedBy ? video.likedBy.length : 0) + (video.comments ? video.comments.length : 0)) / views * 100; document.getElementById('analytics-engagement').innerText = eng.toFixed(1) + '%'; document.getElementById('analytics-modal').classList.add('show'); }
window.openVideoDetails = function(id) { const video = allVideosData.find(v => v.id === id); if (!video) return; document.getElementById('detail-title').innerText = video.title || 'Ohne Titel'; document.getElementById('detail-likes').innerHTML = `<i class="fas fa-heart" style="color: #ff0050;"></i> ${video.likedBy ? video.likedBy.length : 0}`; document.getElementById('detail-views').innerHTML = `<i class="fas fa-play" style="color: #00f2fe;"></i> ${video.views || 0}`; document.getElementById('detail-date').innerHTML = `<i class="fas fa-calendar" style="color: #ffd700;"></i> ${video.timestamp ? timeAgo(video.timestamp) : 'Unbekannt'}`; document.getElementById('detail-desc').innerHTML = formatText(video.description || ''); document.getElementById('video-details-modal').classList.add('show'); }
...
window.openEditVideo = function(videoId) { const video = allVideosData.find(v => v.id === videoId); if (video) { window.currentEditVideoId = videoId; document.getElementById('edit-video-title').value = video.title || ""; document.getElementById('edit-video-desc').value = video.description || ""; 
    const pinBtn = document.getElementById('toggle-pin-btn');
    if(video.pinned) { pinBtn.innerText = "Von Profil lösen"; pinBtn.style.background = "#333"; } else { pinBtn.innerText = "Auf Profil anheften 📌"; pinBtn.style.background = "#00f2fe"; }
...
window.toggleVideoPin = async function() {
    if(!window.currentEditVideoId) return;
    const video = allVideosData.find(v => v.id === window.currentEditVideoId);
    if(video) {
        const newState = !video.pinned;
        try { await updateDoc(doc(db, "videos", video.id), { pinned: newState }); showToast(newState ? "Angeheftet!" : "Gelöst!"); document.getElementById('edit-video-modal').classList.remove('show'); } catch(e) { showCustomAlert("Fehler", "Aktion fehlgeschlagen."); }
    }
};
...
const videoObserver = new IntersectionObserver(entries => {
    entries.forEach(e => {
        ...
        if (e.isIntersecting && document.getElementById('view-feed').classList.contains('active')) {
            ...
            const bgAudio = el.querySelector('.bg-audio-player');
            if (videoPlayer) { 
                ...
                document.querySelectorAll('.bg-audio-player').forEach(otherAudio => { if (otherAudio !== bgAudio && !otherAudio.paused) { otherAudio.pause(); otherAudio.currentTime = 0; } }); 
                
                videoPlayer.muted = window.globalMuted; if(bgAudio) bgAudio.muted = window.globalMuted;
                const playPromise = videoPlayer.play(); 
                if (playPromise !== undefined) { playPromise.catch(error => { videoPlayer.pause(); if(bgAudio) bgAudio.pause(); const container = videoPlayer.closest('.video-inner'); if(container) container.classList.add('is-paused'); }); } 
                if(bgAudio) bgAudio.play().catch(()=>{});
            }
        } else { 
            const videoPlayer = el.querySelector('.video__player'); const bgAudio = el.querySelector('.bg-audio-player');
...
window.openGiftModal = function(videoId) {
    ...
    grid.innerHTML = allGifts.map(g => {
        if(g.reqTier && !checkPhilPlusStatus(g.reqTier)) { return `<div class="gift-card" style="opacity:0.3; cursor:not-allowed;" onclick="showCustomAlert('Plus++ erforderlich', 'Dieses Geschenk ist exklusiv für Phil Shorts++ User!')"><span class="gift-emoji">${g.emoji}</span><span class="gift-name">${g.name}</span><span class="gift-price"><i class="fas fa-lock"></i> Plus++</span></div>`; }
...
function attachInteractionsToVideo(videoContainerEl) {
    const v = videoContainerEl.querySelector('.video__player'); const bgAudio = videoContainerEl.querySelector('.bg-audio-player'); const c = videoContainerEl.querySelector('.carousel-container'); const container = videoContainerEl.querySelector('.video-inner'); videoObserver.observe(videoContainerEl); 
    let lastTap = 0;
    ...
    if (v) {
        ...
        v.addEventListener('click', (e) => { if (handleDoubleTap(e)) return; if (v.paused) { document.querySelectorAll('.video__player').forEach(vid => { if (vid !== v && !vid.paused) vid.pause(); }); document.querySelectorAll('.bg-audio-player').forEach(aud => { if (aud !== bgAudio && !aud.paused) aud.pause(); }); window.globalMuted = false; v.muted = window.globalMuted; if(bgAudio) bgAudio.muted = window.globalMuted; window.updateGlobalVolumeUI(); v.play().catch(err => console.log("Play error:", err)); if(bgAudio) bgAudio.play(); } else { v.pause(); if(bgAudio) bgAudio.pause(); } });
...
window.pinComment = async function(videoId, cId) { if (!currentUser) return; const videoIndex = allVideosData.findIndex(v => v.id === videoId); if (videoIndex === -1) return; const video = allVideosData[videoIndex]; if (currentUser.uid !== video.authorUid || !checkPhilPlusStatus(3)) return; let comments = video.comments || []; const cIndex = comments.findIndex(c => c.cId === cId || c.cId === cId.toString()); if (cIndex === -1) return; const currentState = comments[cIndex].pinned || false; comments.forEach(c => c.pinned = false); comments[cIndex].pinned = !currentState; renderComments(videoId); await updateDoc(doc(db, "videos", videoId), { comments: comments }); showToast(!currentState ? "Kommentar angeheftet!" : "Kommentar gelöst."); };
...
window.submitReply = async function(videoId, cId) { if (!currentUser) return; const input = document.getElementById(`reply-input-${cId}`); const text = input.value.trim(); if (!text) return; const replyId = Date.now().toString(); const reply = { rId: replyId, uid: currentUser.uid, name: currentUser.displayName, username: currentUser.username, pic: currentUser.photoURL, verified: currentUser.verified || false, text: text, likes: [] }; const videoIndex = allVideosData.findIndex(v => v.id === videoId); if (videoIndex > -1) { const comments = allVideosData[videoIndex].comments || []; const cIndex = comments.findIndex((c, idx) => c.cId === cId || idx.toString() === cId); if (cIndex > -1) { if (!comments[cIndex].replies) comments[cIndex].replies = []; comments[cIndex].replies.push(reply); renderComments(videoId); await updateDoc(doc(db, "videos", videoId), { comments: comments }); if (comments[cIndex].uid !== currentUser.uid) addNotification(comments[cIndex].uid, "comment", `hat auf deinen Kommentar geantwortet: "${text}"`, videoId); } } };
...
async function fetchGiphyTrending() { const resultsDiv = document.getElementById('giphy-results'); resultsDiv.innerHTML = '<div style="grid-column: span 2; text-align: center;"><i class="fas fa-spinner fa-spin"></i></div>'; try { const response = await fetch(`https://api.giphy.com/v1/gifs/trending?api_key=${GIPHY_API_KEY}&limit=20&rating=g`); const json = await response.json(); renderGiphyResults(json.data); } catch(e) { resultsDiv.innerHTML = '<div style="grid-column: span 2; text-align: center; color: #888;">Fehler beim Laden von Giphy.</div>'; } }
document.getElementById('giphy-search-input').addEventListener('input', async (e) => { const q = e.target.value.trim(); if(q.length < 2) return fetchGiphyTrending(); try { const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${GIPHY_API_KEY}&q=${encodeURIComponent(q)}&limit=20&rating=g`); const json = await response.json(); renderGiphyResults(json.data); } catch(e) {} });

function renderGiphyResults(gifs) { const resultsDiv = document.getElementById('giphy-results'); resultsDiv.innerHTML = ''; if(!gifs || gifs.length === 0) { resultsDiv.innerHTML = '<div style="grid-column: span 2; text-align: center; color: #888;">Keine GIFs gefunden.</div>'; return; } gifs.forEach(gif => { const url = gif.images.fixed_height.url; resultsDiv.innerHTML += `<div class="gif-item" onclick="selectGifForComment('${url}')"><img src="${url}" alt="GIF"></div>`; }); }
...
document.getElementById('submit-comment').addEventListener('click', async() => { const input = document.getElementById('new-comment-input'); const text = input.value.trim(); if ((!text && !currentPendingGifUrl) || !window.currentCommentVideoId || !currentUser) return; const commentId = Date.now().toString(); const comment = { cId: commentId, uid: currentUser.uid, name: currentUser.displayName, username: currentUser.username, pic: currentUser.photoURL, verified: currentUser.verified || false, text: text, gifUrl: currentPendingGifUrl || null, likes: [], replies: [], creatorHeart: false, pinned: false }; const videoIndex = allVideosData.findIndex(v => v.id === window.currentCommentVideoId); if (videoIndex > -1) { if (!allVideosData[videoIndex].comments) allVideosData[videoIndex].comments = []; allVideosData[videoIndex].comments.push(comment); renderComments(window.currentCommentVideoId); document.querySelectorAll(`.comment-btn[data-id="${window.currentCommentVideoId}"] .comment-count-txt`).forEach(el => el.innerText = allVideosData[videoIndex].comments.length); } input.value = ''; currentPendingGifUrl = null; document.getElementById('pending-gif-preview').style.display = 'none'; await updateDoc(doc(db, "videos", window.currentCommentVideoId), { comments: arrayUnion(comment) }); const targetVidData = allVideosData.find(vd => vd.id === window.currentCommentVideoId); if (targetVidData) addNotification(targetVidData.authorUid, "comment", `hat kommentiert${text ? ': "'+text+'"' : ' mit einem GIF'}`, window.currentCommentVideoId); });

window.toggleFollow = async function(targetUid, element, event) { if (event) event.stopPropagation(); if (!currentUser) return; const userRef = doc(db, "users", currentUser.uid); const targetRef = doc(db, "users", targetUid); const actionBtn = document.getElementById('profile-action-btn'); const statFollowers = document.getElementById('stat-followers'); if (!currentUser.following.includes(targetUid)) { currentUser.following.push(targetUid); localStorage.setItem('phil_session', JSON.stringify(currentUser)); showToast("Gefolgt!"); document.querySelectorAll(`.follow-btn[data-target="${targetUid}"]`).forEach(btn => btn.style.display = 'none'); if (actionBtn && actionBtn.dataset.uid === targetUid) { actionBtn.innerText = "Entfolgen"; actionBtn.classList.add('edit-btn'); statFollowers.innerText = (Number(statFollowers.innerText) || 0) + 1; } await updateDoc(userRef, { following: arrayUnion(targetUid) }); await updateDoc(targetRef, { followers: arrayUnion(currentUser.uid) }); addNotification(targetUid, "follow", "folgt dir jetzt."); } else { currentUser.following = currentUser.following.filter(uid => uid !== targetUid); localStorage.setItem('phil_session', JSON.stringify(currentUser)); showToast("Entfolgt."); document.querySelectorAll(`.follow-btn[data-target="${targetUid}"]`).forEach(btn => btn.style.display = 'block'); if (actionBtn && actionBtn.dataset.uid === targetUid) { actionBtn.innerText = "Folgen"; actionBtn.classList.remove('edit-btn'); statFollowers.innerText = Math.max(0, (Number(statFollowers.innerText) || 0) - 1); } await updateDoc(userRef, { following: arrayRemove(targetUid) }); await updateDoc(targetRef, { followers: arrayRemove(currentUser.uid) }); } };

window.renderBlockedUsersList = function() { const list = document.getElementById('blocked-users-list'); if(!list) return; if(!currentUser || !currentUser.blockedUsers || currentUser.blockedUsers.length === 0) { list.innerHTML = '<p style="color:#888; font-size:13px; text-align:center;">Keine blockierten Nutzer.</p>'; return; } list.innerHTML = ''; currentUser.blockedUsers.forEach(uid => { const u = allKnownUsers.find(x => x.uid === uid); if(u) { list.innerHTML += `<div class="blocked-user-item"><div style="display:flex; align-items:center; gap:10px;"><img src="${u.photoURL}" style="width:30px; height:30px; border-radius:50%; object-fit:cover;"><span style="font-size:14px; font-weight:bold; color:white;">@${u.username}</span></div><button class="profile-action-btn edit-btn" onclick="toggleBlockUser('${uid}')" style="min-height:30px; font-size:12px; padding:0 10px; background:#ff4444; color:white; border:none;">Entblocken</button></div>`; } }); };
window.toggleBlockUser = async function(targetUid) { if(!currentUser) return; const isBlocked = currentUser.blockedUsers && currentUser.blockedUsers.includes(targetUid); try { if(isBlocked) { currentUser.blockedUsers = currentUser.blockedUsers.filter(id => id !== targetUid); await updateDoc(doc(db, "users", currentUser.uid), { blockedUsers: arrayRemove(targetUid) }); showToast("Nutzer entblockt."); } else { if(!currentUser.blockedUsers) currentUser.blockedUsers = []; currentUser.blockedUsers.push(targetUid); if(currentUser.following.includes(targetUid)) { currentUser.following = currentUser.following.filter(id => id !== targetUid); await updateDoc(doc(db, "users", currentUser.uid), { following: arrayRemove(targetUid) }); } await updateDoc(doc(db, "users", currentUser.uid), { blockedUsers: arrayUnion(targetUid) }); showToast("Nutzer blockiert. Lade neu..."); setTimeout(() => window.location.reload(), 1500); } localStorage.setItem('phil_session', JSON.stringify(currentUser)); if(document.getElementById('view-profile').classList.contains('active')) { openProfile(targetUid); } renderBlockedUsersList(); } catch(e) { showCustomAlert("Fehler", "Blockieren fehlgeschlagen."); } };
...
window.renderProfileGrid = function(targetUid) {
    ...
    if(window.currentProfileTab === 'grid') { videosToRender = allVideosData.filter(v => v.authorUid === targetUid && !blocked.includes(v.authorUid)); videosToRender.sort((a,b) => (b.pinned === true) - (a.pinned === true)); } 
    ...
    else { videosToRender.forEach(v => { const previewSrc = v.mediaType === 'images' && v.urls ? v.urls[0] : `${v.url}#t=0.5`; const mediaTag = v.mediaType === 'images' ? `<img src="${previewSrc}" style="width:100%; height:100%; object-fit:cover;">` : `<video src="${previewSrc}" muted playsinline style="width:100%; height:100%; object-fit:cover;"></video>`; const icon = v.mediaType === 'images' ? 'fa-images' : 'fa-play'; const pinIcon = v.pinned && window.currentProfileTab === 'grid' ? '<i class="fas fa-thumbtack" style="color:#ffd700; margin-right:5px;"></i>' : ''; grid.innerHTML += `<div class="grid-item" onclick="jumpToVideo('${v.id}')">${mediaTag}<div class="grid-views">${pinIcon}<i class="fas ${icon}"></i> ${v.likedBy ? v.likedBy.length : 0}</div></div>`; }); }
}
...
window.openProfile = async function(targetUid) {
    ...
    currentProfileUnsubscribe = onSnapshot(doc(db, "users", targetUid), (docSnap) => {
        ...
        if(targetUser.philPlusUntil && targetUser.philPlusUntil > Date.now() && (targetUser.philPlusTier || 1) >= 1) { 
            nameClass = "name-phil-plus"; document.getElementById('phil-plus-badge-container').style.display = 'block'; 
            let tierText = "Phil Shorts+"; if(targetUser.philPlusTier === 2) tierText = "Phil Shorts++";
        ...
        if(targetUser.philPlusUntil && targetUser.philPlusUntil > Date.now() && targetUser.philPlusTier === 3) {
            ...
            if(targetUser.profileSong && document.getElementById('profile-audio-player').src !== targetUser.profileSong) { document.getElementById('profile-audio-player').src = targetUser.profileSong; document.getElementById('profile-audio-player').volume = 0.5; document.getElementById('profile-audio-player').play().catch(e => console.log("Autoplay blockiert")); }
        }
        ...
        document.getElementById('stat-streak').innerText = (targetUser.streak || 0) + " 🔥";
        ...
        if (currentUser && targetUid === currentUser.uid) { 
            ...
            settingsIcon.style.display = 'block'; settingsIcon.onclick = () => { updateNotifUI(); renderBlockedUsersList(); document.getElementById('app-settings-modal').classList.add('show'); document.getElementById('settings-close-friends').value = (currentUser.closeFriends || []).join(', '); }; adminDashboardBtn.style.display = (currentUser.email === "schleimyverteilung@gmail.com" || currentUser.isAdmin) ? 'block' : 'none'; privateStats.style.display = 'block'; document.getElementById('my-coins').innerText = targetUser.coins || 0; document.getElementById('my-views').innerText = targetUser.profileViews || 0; 
        } 
        else { 
    ...
    if (currentUser && targetUid !== currentUser.uid) { if(!checkPhilPlusStatus(3)) { updateDoc(doc(db, "users", targetUid), { profileViews: increment(1) }).catch(e => {}); } }
};
...
function updateCustomBorderPreview() { const c1 = document.getElementById('cb-color1').value; const c2 = document.getElementById('cb-color2').value; const grad = document.getElementById('cb-grad-toggle').checked; if(grad) { document.getElementById('cb-color2-container').style.display = 'block'; } else { document.getElementById('cb-color2-container').style.display = 'none'; } const preview = document.getElementById('custom-border-preview'); applyBorderStyles(preview, 'custom', { c1, c2, grad }); }
...
window.saveCustomBorder = async function() { if(!currentUser || !checkPhilPlusStatus(3)) return; const c1 = document.getElementById('cb-color1').value; const c2 = document.getElementById('cb-color2').value; const grad = document.getElementById('cb-grad-toggle').checked; try { currentUser.activeBorder = 'custom'; currentUser.customBorder = { c1, c2, grad }; await updateDoc(doc(db, "users", currentUser.uid), { activeBorder: 'custom', customBorder: { c1, c2, grad } }); document.getElementById('custom-border-modal').classList.remove('show'); renderShopBorders(); showToast("Custom Border ausgerüstet!"); } catch(e) { showCustomAlert("Fehler", "Konnte nicht gespeichert werden."); } }

window.pendingSub = null;
window.buyPhilPlus = async function(days, cost, tier) { 
    if(!currentUser) return; 
    if(checkPhilPlusStatus(1)) { window.pendingSub = { days, cost, tier }; let msg = `Du besitzt aktuell Stufe ${currentUser.philPlusTier}. Möchtest du dein neues Abo (Stufe ${tier}) kaufen und das aktuelle überschreiben, oder dein Abo komplett löschen?`; if(currentUser.philPlusTier === tier) { msg = `Du hast bereits Stufe ${tier}. Möchtest du dein Abo um ${days} Tage verlängern, oder das Abo abbestellen?`; } document.getElementById('sub-conflict-msg').innerText = msg; document.getElementById('shop-modal').classList.remove('show'); document.getElementById('sub-conflict-modal').classList.add('show'); return; }
...
window.confirmSubPurchase = function() { if(!window.pendingSub) return; const { days, cost, tier } = window.pendingSub; if(currentUser.coins < cost) { showCustomAlert("Zu wenig Coins", "Du hast nicht genug Coins dafür."); document.getElementById('sub-conflict-modal').classList.remove('show'); return; } document.getElementById('sub-conflict-modal').classList.remove('show'); executeSubPurchase(days, cost, tier); }
window.cancelSubscription = async function() { if(!currentUser) return; if(confirm("Möchtest du dein Phil Shorts+ Abo WIRKLICH endgültig löschen? (Keine Rückerstattung!)")) { try { currentUser.philPlusUntil = 0; currentUser.philPlusTier = 0; await updateDoc(doc(db, "users", currentUser.uid), { philPlusUntil: 0, philPlusTier: 0 }); document.getElementById('sub-conflict-modal').classList.remove('show'); showToast("Abo erfolgreich gelöscht."); initLiveUser(); } catch(e) { showCustomAlert("Fehler", "Konnte nicht gekündigt werden."); } } }
...
document.getElementById('submit-story-upload').addEventListener('click', async() => { 
    ...
    try { 
        // Firebase Storage statt Cloudinary
...
window.viewUserStory = function(index = 0) {
    ...
    if(index >= profileUserStories.length) { closeStoryViewer(); return; } if(index < 0) index = 0;
...
document.getElementById('save-settings-btn').addEventListener('click', async() => {
    const newDisplayName = document.getElementById('edit-displayname-input').value.trim(); const newUsernameRaw = document.getElementById('edit-username-input').value.trim(); const newUsername = newUsernameRaw.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase(); const newBio = document.getElementById('edit-bio-input').value.trim(); const newPic = document.getElementById('edit-pic-input').value.trim() || currentUser.photoURL; const newSong = document.getElementById('edit-song-input').value.trim(); const newColor = document.getElementById('edit-color-input').value;
    const newIg = document.getElementById('edit-social-ig').value.trim().replace('@', ''); const newYt = document.getElementById('edit-social-yt').value.trim().replace('@', ''); const newTt = document.getElementById('edit-social-tt').value.trim().replace('@', ''); const newTw = document.getElementById('edit-social-tw').value.trim().replace('@', '');
    ...
    try {
        ...
        let cfRaw = document.getElementById('settings-close-friends').value;
        let cfArray = cfRaw.split(',').map(s => s.trim().replace('@','')).filter(s => s !== "");
        updates.closeFriends = cfArray;
        currentUser.closeFriends = cfArray;
...
document.getElementById('search-input').addEventListener('input', (e) => {
    ...
    const matchedVideos = allVideosData.filter(v => !v.closeFriendsOnly && !blocked.includes(v.authorUid) && ((v.description || "").toLowerCase().includes(query) || (v.authorName || "").toLowerCase().includes(query) || (v.title || "").toLowerCase().includes(query)));
...
function initInboxChats() {
    ...
    inboxChatsUnsubscribe = onSnapshot(collection(db, "chats"), (snapshot) => {
        let blocked = (currentUser && currentUser.blockedUsers) ? currentUser.blockedUsers : [];
        let chats = []; snapshot.forEach(doc => { const chat = doc.data(); if (chat.participants && chat.participants.includes(currentUser.uid)) { const partnerUid = chat.participants.find(uid => uid !== currentUser.uid); if(!blocked.includes(partnerUid)) { chats.push({ id: doc.id, ...chat }); } } }); chats.sort((a, b) => b.lastMessageTime - a.lastMessageTime); msgBox.innerHTML = '';
        if (chats.length === 0) { msgBox.innerHTML = '<div class="empty-state" style="height:100%;"><p>Keine Nachrichten vorhanden</p></div>'; return; }
        chats.forEach(chat => { const partnerUid = chat.participants.find(uid => uid !== currentUser.uid); const partner = chat.users[partnerUid]; if (!partner) return; const nUser = getUserData(partnerUid, partner.name, partner.name, partner.pic, false); const safeName = nUser.username.replace(/'/g, "\\'"); const isVerif = getVerifiedBadge(nUser.verified); let nameClass = nUser.philPlusUntil && nUser.philPlusUntil > Date.now() && nUser.philPlusTier >= 1 ? "name-phil-plus" : ""; let previewText = chat.lastMessage; if(previewText && previewText.startsWith('[IMAGE]')) previewText = "📸 Bild gesendet"; msgBox.innerHTML += `<div class="inbox-msg" onclick="openDM('${partnerUid}', '${safeName}', '${nUser.pic}')"><img src="${nUser.pic}" class="chat-avatar live-pic-${partnerUid}" style="flex-shrink:0;"><div style="flex:1; min-width:0;"><span class="chat-username" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"><span class="live-name-${partnerUid} ${nameClass}">${nUser.displayName}${isVerif}</span> <span class="live-username-${partnerUid}" style="color:#888; font-weight:normal; font-size:12px;">@${nUser.username}</span></span><div class="chat-bubble" style="background: transparent; padding: 0; color: #888; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${formatText(previewText) || 'Neuer Chat...'}</div><div class="chat-time" style="font-size: 11px; color: #666; margin-top: 4px;">${timeAgo(chat.lastMessageTime)}</div></div></div>`; });
    });
}
...
window.initSupportTickets = function() {
    ...
    if (supportUnsubscribe) supportUnsubscribe(); let reportQuery = query(collection(db, "reports"), orderBy("timestamp", "desc"));
    supportUnsubscribe = onSnapshot(reportQuery, (snapshot) => {
        supportBox.innerHTML = ''; let foundAny = false;
        snapshot.forEach(docSnap => {
            const ticket = docSnap.data(); if (!isAdmin && ticket.uid !== currentUser.uid) return; foundAny = true; const ticketId = docSnap.id; const isVip = ticket.hasPlus ? 'vip' : ''; let plusText = "PLUS"; if(ticket.tier === 2) plusText = "PLUS++"; if(ticket.tier === 3) plusText = "PLUS+++"; const vipBadge = ticket.hasPlus ? `<span class="phil-plus-badge" style="font-size:9px; margin-left:5px;">${plusText}</span>` : ''; const uData = getUserData(ticket.uid, ticket.name, ticket.name, 'https://api.dicebear.com/7.x/avataaars/svg?seed=fallback', false); const safeName = uData.username.replace(/'/g, "\\'");
            let adminButtons = ''; if (isAdmin) { if (ticket.status === 'closed') { adminButtons = `<button class="profile-action-btn edit-btn" onclick="deleteTicket(event, '${ticketId}')" style="min-height:26px; font-size:11px; background:transparent; border:1px solid #ff4444; color:#ff4444; padding:0 8px;"><i class="fas fa-trash"></i> Löschen</button>`; } else { adminButtons = `<button class="profile-action-btn edit-btn" onclick="resolveTicket(event, '${ticketId}')" style="min-height:26px; font-size:11px; background:transparent; border:1px solid #ffd700; color:#ffd700; padding:0 8px;"><i class="fas fa-lock"></i> Schließen</button>`; } }
            supportBox.innerHTML += `<div class="support-ticket ${isVip}" onclick="openTicketChat('${ticketId}', '${safeName}', '${ticket.uid}')" style="cursor:pointer; display:flex; flex-direction:column; gap:8px;"><div style="display:flex; justify-content:space-between; align-items:center;"><strong style="color:white; font-size:14px;">@${uData.username} ${vipBadge}</strong><span style="color:#888; font-size:11px;">${timeAgo(ticket.timestamp)}</span></div><div style="display:flex; justify-content:space-between; align-items:center;"><div style="font-size:12px; color:#aaa;"><i class="fas fa-ticket-alt"></i> Status: <span style="color:${ticket.status === 'closed' ? '#ff4444' : '#39ff14'}; font-weight:bold;">${ticket.status === 'closed' ? 'Geschlossen' : 'Offen'}</span></div>${adminButtons}</div><p style="font-size:14px; color:#ddd; background:rgba(0,0,0,0.3); padding:10px; border-radius:8px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">Klicken um Chat zu öffnen</p></div>`;
        });
...
window.resolveTicket = async function(event, ticketId) { event.stopPropagation(); if(confirm("Ticket als erledigt markieren und schließen?")) { try { await updateDoc(doc(db, "reports", ticketId), { status: 'closed' }); showToast("Ticket geschlossen."); } catch(e) { showCustomAlert("Fehler", "Konnte nicht geschlossen werden."); } } };
window.deleteTicket = async function(event, ticketId) { event.stopPropagation(); if(confirm("Ticket endgültig löschen?")) { try { await deleteDoc(doc(db, "reports", ticketId)); showToast("Ticket gelöscht."); } catch(e) { showCustomAlert("Fehler", "Konnte nicht gelöscht werden."); } } };
...
window.openTicketChat = async function(ticketId, username, ticketOwnerUid) {
    if (!currentUser) return; window.currentActiveTicketId = ticketId; document.getElementById('ticket-title').innerText = "Ticket: @" + username; switchView('ticket'); const ticketBox = document.getElementById('ticket-box'); ticketBox.innerHTML = '<div class="loading-screen"><i class="fas fa-circle-notch fa-spin"></i></div>'; const isAdmin = (currentUser.email === "schleimyverteilung@gmail.com" || currentUser.isAdmin);
    ...
    currentTicketSnapshot = onSnapshot(query(collection(db, `reports/${ticketId}/messages`), orderBy("timestamp", "asc")), (snapshot) => {
        ticketBox.innerHTML = ''; if (snapshot.empty) { ticketBox.innerHTML = '<div class="empty-state" style="height:100%;"><p>Keine Nachrichten</p></div>'; } else { snapshot.forEach(docSnap => { const msg = docSnap.data(); const isMe = msg.senderUid === currentUser.uid ? 'me' : ''; const isSupportSender = msg.senderUid !== ticketOwnerUid; const pic = isSupportSender ? 'https://api.dicebear.com/7.x/avataaars/svg?seed=admin' : (msg.senderUid === currentUser.uid ? currentUser.photoURL : `https://api.dicebear.com/7.x/avataaars/svg?seed=${msg.senderUid}`); let bg = isMe ? '#ff0050' : '#333'; let adminLabel = isSupportSender ? '<div style="font-size:10px; color:#ffd700; margin-bottom:4px;"><i class="fas fa-shield-alt"></i> Support Team</div>' : ''; ticketBox.innerHTML += `<div class="chat-msg ${isMe}"><img src="${pic}" class="chat-avatar" style="flex-shrink:0;"><div style="min-width:0; max-width: 100%;"><div class="chat-bubble" style="background:${bg}; border-color:${bg};">${adminLabel}${formatText(msg.text)}</div><div class="chat-time" style="font-size: 10px; color: #666; margin-top: 4px; text-align: ${isMe ? 'right' : 'left'};">${timeAgo(msg.timestamp)}</div></div></div>`; }); } ticketBox.scrollTop = ticketBox.scrollHeight;
    });
    currentTicketMetaSnapshot = onSnapshot(doc(db, "reports", ticketId), (docSnap) => {
        if(docSnap.exists()) { const tData = docSnap.data(); const statEl = document.getElementById('ticket-status'); if(tData.status === 'closed') { statEl.innerText = "Geschlossen"; statEl.style.background = "#ff4444"; statEl.style.color = "white"; document.getElementById('ticket-input-area').style.display = 'none'; document.getElementById('admin-close-ticket-btn').style.display = 'none'; } else { statEl.innerText = "Offen"; statEl.style.background = "#39ff14"; statEl.style.color = "black"; document.getElementById('ticket-input-area').style.display = 'flex'; if(isAdmin) document.getElementById('admin-close-ticket-btn').style.display = 'block'; else document.getElementById('admin-close-ticket-btn').style.display = 'none'; } } else { document.getElementById('ticket-status').innerText = "Gelöscht"; document.getElementById('ticket-status').style.background = "#ff4444"; document.getElementById('ticket-input-area').style.display = 'none'; document.getElementById('admin-close-ticket-btn').style.display = 'none'; }
    });
};
...
document.getElementById('admin-close-ticket-btn').addEventListener('click', async() => { if(!window.currentActiveTicketId) return; if(confirm("Support Ticket schließen? (Nutzer kann nicht mehr antworten)")) { await updateDoc(doc(db, "reports", window.currentActiveTicketId), { status: 'closed' }); showToast("Ticket geschlossen."); } });
...
window.openDM = async function(targetUid, targetName, targetPic) {
    ...
    let statusHtml = ''; if(nUser.lastActive) { let diff = Date.now() - nUser.lastActive; if(diff < 5 * 60000) statusHtml = '<span style="color:#39ff14;">🟢 Online</span>'; else statusHtml = 'Zuletzt online: ' + timeAgo(nUser.lastActive); } document.getElementById('dm-status-span').innerHTML = statusHtml;
    if(checkPhilPlusStatus(3)) document.getElementById('dm-img-btn').style.display = 'block'; else document.getElementById('dm-img-btn').style.display = 'none';
    ...
    currentDMSnapshot = onSnapshot(query(collection(db, `chats/${window.currentChatId}/messages`), orderBy("timestamp", "asc")), (snapshot) => {
        ...
        else { snapshot.forEach(doc => { const msg = doc.data(); const isMe = msg.senderUid === currentUser.uid ? 'me' : ''; const pic = isMe ? currentUser.photoURL : targetPic; let readReceipt = ''; if(isMe && checkPhilPlusStatus(2)) readReceipt = `<span style="font-size:10px; color:#00f2fe; margin-left:5px;">✓✓</span>`; let extraClass = ''; if(isMe && checkPhilPlusStatus(2)) extraClass = 'gold-bubble'; let bubbleContent = formatText(msg.text); if(msg.text && msg.text.startsWith('[IMAGE]')) { const imgUrl = msg.text.replace('[IMAGE]', '').trim(); bubbleContent = `<img src="${imgUrl}" style="max-width: 200px; border-radius: 10px;">`; } dmBox.innerHTML += `<div class="chat-msg ${isMe}"><img src="${pic}" class="chat-avatar" style="flex-shrink:0;"><div style="min-width:0; max-width: 100%;"><div class="chat-bubble ${extraClass}">${bubbleContent}</div><div class="chat-time" style="font-size: 10px; color: #666; margin-top: 4px; text-align: ${isMe ? 'right' : 'left'};">${timeAgo(msg.timestamp)}${readReceipt}</div></div></div>`; }); }
...
document.getElementById('dm-file-upload').addEventListener('change', async(e) => {
    ...
    try {
        const storageRef = ref(storage, `dm_images/${currentUser.uid}_${Date.now()}_${file.name}`);
        await uploadBytesResumable(storageRef, file);
        const finalUrl = await getDownloadURL(storageRef);
...
// --- DRAG & DROP LOGIK ---
document.getElementById('btn-toggle-grid').addEventListener('click', (e) => { dragState.gridVisible = !dragState.gridVisible; document.getElementById('editor-grid').style.display = dragState.gridVisible ? 'block' : 'none'; e.target.innerHTML = dragState.gridVisible ? '<i class="fas fa-border-all"></i> Raster: An' : '<i class="fas fa-border-all"></i> Raster: Aus'; });

function startDrag(e, id, isVideo = false) { 
    if(e.target.tagName.toLowerCase() === 'input' || e.target.classList.contains('resize-handle')) return; e.preventDefault(); e.stopPropagation(); 
    dragState.activeId = id; dragState.isVideo = isVideo; if(isVideo) selectProOverlay(id); else selectText(id); 
    const clientX = e.touches ? e.touches[0].clientX : e.clientX; const clientY = e.touches ? e.touches[0].clientY : e.clientY; dragState.startX = clientX; dragState.startY = clientY; 
    let obj; if(isVideo) obj = proEditorState.overlays.find(t => t.id === id); else obj = imageEditorState.edits[imageEditorState.currentIndex].find(t => t.id === id);
    if(obj) { dragState.initialObjX = obj.x; dragState.initialObjY = obj.y; }
}

function startResize(e, id, pos, isVideo = false) { 
    e.preventDefault(); e.stopPropagation(); dragState.activeResizeId = id; dragState.isVideo = isVideo; dragState.activeResizePos = pos; 
    if(isVideo) selectProOverlay(id); else selectText(id); 
    const clientX = e.touches ? e.touches[0].clientX : e.clientX; const clientY = e.touches ? e.touches[0].clientY : e.clientY; dragState.startX = clientX; dragState.startY = clientY; 
    let obj; if(isVideo) obj = proEditorState.overlays.find(t => t.id === id); else obj = imageEditorState.edits[imageEditorState.currentIndex].find(t => t.id === id);
    if(obj) { dragState.startSize = parseFloat(obj.size); const workspaceId = isVideo ? 'pro-editor-workspace' : 'editor-workspace'; const workspaceRect = document.getElementById(workspaceId).getBoundingClientRect(); const mouseX = clientX - workspaceRect.left; const mouseY = clientY - workspaceRect.top; dragState.startDist = Math.sqrt(Math.pow(mouseX - obj.x, 2) + Math.pow(mouseY - obj.y, 2)); }
}

function dragMove(e) {
    if (dragState.activeId) { 
        e.preventDefault(); const clientX = e.touches ? e.touches[0].clientX : e.clientX; const clientY = e.touches ? e.touches[0].clientY : e.clientY; const dx = clientX - dragState.startX; const dy = clientY - dragState.startY; let rawX = dragState.initialObjX + dx; let rawY = dragState.initialObjY + dy; 
        const workspaceId = dragState.isVideo ? 'pro-editor-workspace' : 'editor-workspace'; const ws = document.getElementById(workspaceId);
        let snappedX = false; let snappedY = false;
        if (Math.abs(rawX - (ws.clientWidth / 2)) < 15) { rawX = ws.clientWidth / 2; snappedX = true; } if (Math.abs(rawY - (ws.clientHeight / 2)) < 15) { rawY = ws.clientHeight / 2; snappedY = true; }
        const snapVId = dragState.isVideo ? 'pro-snap-v' : 'img-snap-v'; const snapHId = dragState.isVideo ? 'pro-snap-h' : 'img-snap-h'; document.getElementById(snapVId).style.display = snappedX ? 'block' : 'none'; document.getElementById(snapHId).style.display = snappedY ? 'block' : 'none';
        if (!dragState.isVideo && dragState.gridVisible) { rawX = Math.round(rawX / 20) * 20; rawY = Math.round(rawY / 20) * 20; } 
        let obj; if(dragState.isVideo) obj = proEditorState.overlays.find(t => t.id === dragState.activeId); else obj = imageEditorState.edits[imageEditorState.currentIndex].find(t => t.id === dragState.activeId);
        if (obj) { obj.x = rawX; obj.y = rawY; const prefix = dragState.isVideo ? 'pro-drag-txt-' : 'drag-txt-'; const el = document.getElementById(prefix + obj.id); if(el) { el.style.left = obj.x + 'px'; el.style.top = obj.y + 'px'; } } 
    } else if (dragState.activeResizeId) { 
        e.preventDefault(); const clientX = e.touches ? e.touches[0].clientX : e.clientX; const clientY = e.touches ? e.touches[0].clientY : e.clientY; const workspaceId = dragState.isVideo ? 'pro-editor-workspace' : 'editor-workspace'; const workspaceRect = document.getElementById(workspaceId).getBoundingClientRect(); const mouseX = clientX - workspaceRect.left; const mouseY = clientY - workspaceRect.top; 
        let obj; if(dragState.isVideo) obj = proEditorState.overlays.find(t => t.id === dragState.activeResizeId); else obj = imageEditorState.edits[imageEditorState.currentIndex].find(t => t.id === dragState.activeResizeId);
        if (obj) { const currentDist = Math.sqrt(Math.pow(mouseX - obj.x, 2) + Math.pow(mouseY - obj.y, 2)); if (dragState.startDist > 0) { let newSize = Math.max(15, dragState.startSize * (currentDist / dragState.startDist)); obj.size = newSize; const prefix = dragState.isVideo ? 'pro-drag-txt-' : 'drag-txt-'; const el = document.getElementById(prefix + obj.id); if(el) { if(obj.type === 'sticker' || obj.type === 'blur'){ el.children[0].style.width = newSize+'px'; if(obj.type === 'blur') el.children[0].style.height = newSize+'px'; } else { el.querySelector('.text-content').style.fontSize = newSize + 'px'; } } const sizeCtrl = dragState.isVideo ? 'pro-text-ctrl-size' : 'text-ctrl-size'; document.getElementById(sizeCtrl).value = newSize; } } 
    }
}
function endDrag() { dragState.activeId = null; dragState.activeResizeId = null; document.getElementById('pro-snap-v').style.display = 'none'; document.getElementById('pro-snap-h').style.display = 'none'; document.getElementById('img-snap-v').style.display = 'none'; document.getElementById('img-snap-h').style.display = 'none'; }
...
function renderEditorImage(index) { if (imageEditorState.images.length === 0) return; document.getElementById('editor-bg').src = imageEditorState.images[index]; document.getElementById('editor-img-counter').innerText = `${index + 1} / ${imageEditorState.images.length}`; const layer = document.getElementById('editor-layer'); layer.innerHTML = ''; imageEditorState.activeTextId = null; document.getElementById('text-controls').style.display = 'none'; imageEditorState.edits[index].forEach(obj => createDOMTextElement(obj, false)); }

function createDOMTextElement(obj, isVideo = false) {
    const layerId = isVideo ? 'pro-editor-layer' : 'editor-layer'; const prefix = isVideo ? 'pro-drag-txt-' : 'drag-txt-';
    const layer = document.getElementById(layerId); const wrapper = document.createElement('div'); wrapper.id = prefix + obj.id; wrapper.className = 'draggable-text'; wrapper.style.left = obj.x + 'px'; wrapper.style.top = obj.y + 'px'; wrapper.style.transform = `translate(-50%, -50%) rotate(${obj.rotation}deg)`; 
    if(obj.type === 'sticker') { const imgEl = document.createElement('img'); imgEl.className = 'sticker-content'; imgEl.src = obj.src; imgEl.style.width = obj.size + 'px'; imgEl.style.height = 'auto'; imgEl.style.pointerEvents = 'none'; wrapper.appendChild(imgEl); } else if (obj.type === 'blur') { const blurEl = document.createElement('div'); blurEl.className = 'blur-box-content'; blurEl.style.width = obj.size + 'px'; blurEl.style.height = obj.size + 'px'; wrapper.appendChild(blurEl); } else { const textEl = document.createElement('div'); textEl.className = 'text-content'; textEl.innerText = obj.text; textEl.style.fontSize = obj.size + 'px'; textEl.style.color = obj.color; textEl.style.fontFamily = obj.font || 'Arial, sans-serif'; wrapper.appendChild(textEl); }
...
function selectText(id) { imageEditorState.activeTextId = id; document.querySelectorAll('#editor-layer .draggable-text').forEach(el => el.classList.remove('active')); const obj = imageEditorState.edits[imageEditorState.currentIndex].find(t => t.id === id); if(!obj) return; document.getElementById('drag-txt-' + id).classList.add('active'); document.getElementById('text-controls').style.display = 'block'; document.getElementById('text-ctrl-input').value = obj.text || ''; document.getElementById('text-ctrl-size').value = obj.size; document.getElementById('text-ctrl-rot').value = obj.rotation; document.getElementById('text-ctrl-font').value = obj.font || 'Arial, sans-serif'; if(obj.type === 'sticker') { document.getElementById('text-ctrl-input').style.display = 'none'; document.getElementById('img-font-group').style.display = 'none'; document.getElementById('img-color-group').style.display = 'none'; } else { document.getElementById('text-ctrl-input').style.display = 'block'; document.getElementById('img-font-group').style.display = 'block'; document.getElementById('img-color-group').style.display = 'block'; } }
...
document.getElementById('sticker-upload-input').addEventListener('change', (e) => { const file = e.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = function(event) { const workspace = document.getElementById('editor-workspace'); const newObj = { id: Date.now(), type: 'sticker', src: event.target.result, x: workspace.clientWidth / 2, y: workspace.clientHeight / 2, size: 100, rotation: 0 }; imageEditorState.edits[imageEditorState.currentIndex].push(newObj); createDOMTextElement(newObj, false); selectText(newObj.id); document.getElementById('sticker-upload-input').value = ''; }; reader.readAsDataURL(file); });
...
function selectProOverlay(id) { 
    proEditorState.activeTextId = id; document.querySelectorAll('#pro-editor-layer .draggable-text').forEach(el => el.classList.remove('active')); const obj = proEditorState.overlays.find(t => t.id === id); if(!obj) return; 
    document.getElementById('pro-drag-txt-' + id).classList.add('active'); document.getElementById('pro-text-controls').style.display = 'block'; document.getElementById('pro-text-ctrl-input').value = obj.text || ''; document.getElementById('pro-text-ctrl-size').value = obj.size; document.getElementById('pro-text-ctrl-rot').value = obj.rotation; document.getElementById('pro-text-ctrl-font').value = obj.font || 'Arial, sans-serif'; document.getElementById('pro-overlay-start').value = obj.start || 0; document.getElementById('pro-overlay-end').value = obj.end || (proEditorState.end || 10);
    if(obj.type === 'sticker' || obj.type === 'blur') { document.getElementById('pro-text-ctrl-input').style.display = 'none'; document.getElementById('pro-font-group').style.display = 'none'; document.getElementById('pro-color-group').style.display = 'none'; } else { document.getElementById('pro-text-ctrl-input').style.display = 'block'; document.getElementById('pro-font-group').style.display = 'flex'; document.getElementById('pro-color-group').style.display = 'block'; }
}

document.getElementById('pro-editor-workspace').addEventListener('click', (e) => { if (!e.target.closest('.draggable-text')) { proEditorState.activeTextId = null; document.querySelectorAll('#pro-editor-layer .draggable-text').forEach(el => el.classList.remove('active')); document.getElementById('pro-text-controls').style.display = 'none'; } });
document.getElementById('btn-pro-add-text').addEventListener('click', () => { const workspace = document.getElementById('pro-editor-workspace'); const newObj = { id: Date.now(), type: 'text', text: "Video Text", x: workspace.clientWidth / 2, y: workspace.clientHeight / 2, size: 40, rotation: 0, color: '#ffffff', font: 'Arial, sans-serif', start: 0, end: proEditorState.end || 10 }; proEditorState.overlays.push(newObj); createDOMTextElement(newObj, true); selectProOverlay(newObj.id); });
document.getElementById('btn-pro-add-blur').addEventListener('click', () => { const workspace = document.getElementById('pro-editor-workspace'); const newObj = { id: Date.now(), type: 'blur', x: workspace.clientWidth / 2, y: workspace.clientHeight / 2, size: 100, rotation: 0, start: 0, end: proEditorState.end || 10 }; proEditorState.overlays.push(newObj); createDOMTextElement(newObj, true); selectProOverlay(newObj.id); });

document.getElementById('pro-sticker-upload-input').addEventListener('change', (e) => { const file = e.target.files[0]; if(!file) return; const reader = new FileReader(); reader.onload = function(event) { const workspace = document.getElementById('pro-editor-workspace'); const newObj = { id: Date.now(), type: 'sticker', src: event.target.result, x: workspace.clientWidth / 2, y: workspace.clientHeight / 2, size: 100, rotation: 0, start: 0, end: proEditorState.end || 10 }; proEditorState.overlays.push(newObj); createDOMTextElement(newObj, true); selectProOverlay(newObj.id); document.getElementById('pro-sticker-upload-input').value = ''; }; reader.readAsDataURL(file); });
...
document.getElementById('up-file').addEventListener('change', async function(e) {
    ...
    const advancedVideoUi = document.getElementById('video-advanced-editor'); const proPreview = document.getElementById('pro-video-source'); const proTrimStart = document.getElementById('pro-trim-start'); const proTrimEnd = document.getElementById('pro-trim-end'); const advancedImageUi = document.getElementById('image-advanced-editor'); 

    if (!files || files.length === 0) { txt.innerText = "Video oder Bilder auswählen"; icon.className = "fas fa-cloud-upload-alt"; icon.style.color = "#aaa"; advancedVideoUi.style.display = 'none'; advancedImageUi.style.display = 'none'; return; }
    const isVideo = files[0].type.startsWith('video/');
    if (isVideo) {
        txt.innerText = files[0].name; icon.className = "fas fa-video"; icon.style.color = "#00f2fe"; advancedVideoUi.style.display = 'block'; advancedImageUi.style.display = 'none';
        const url = URL.createObjectURL(files[0]); proPreview.src = url; 
        proPreview.onloadedmetadata = () => { 
            const dur = proPreview.duration; 
            if(dur > 180 && !checkPhilPlusStatus(2)) { showCustomAlert("Zu lang", "Videos über 3 Minuten erfordern Phil Shorts++!"); document.getElementById('up-file').value = ''; txt.innerText = "Video oder Bilder auswählen"; icon.className = "fas fa-cloud-upload-alt"; icon.style.color = "#aaa"; advancedVideoUi.style.display = 'none'; return; }
            document.getElementById('pro-time-total').innerText = dur.toFixed(1); proTrimStart.max = dur; proTrimEnd.max = dur; proTrimEnd.value = dur; document.getElementById('pro-val-end').innerText = dur.toFixed(1); document.getElementById('pro-overlay-start').max = dur; document.getElementById('pro-overlay-end').max = dur;
            proEditorState.end = dur; proEditorState.start = 0; proEditorState.filter = 'none'; proEditorState.speed = 1; proEditorState.ratio = 'original'; proEditorState.audioFileUrl = null; proEditorState.origVol = 1; proEditorState.bgVol = 0.5; proEditorState.overlays = []; proEditorState.activeTextId = null;
            document.getElementById('pro-editor-layer').innerHTML = ''; document.getElementById('pro-text-controls').style.display = 'none'; document.getElementById('pro-audio-orig-vol').value = 1; document.getElementById('pro-audio-bg-vol').value = 0.5; document.getElementById('pro-audio-bg-name').innerText = ""; document.getElementById('pro-speed-ctrl').value = "1"; document.getElementById('pro-ratio-ctrl').value = "original"; document.getElementById('pro-audio-controls-extended').style.display = 'none'; proBgAudio.src = "";
            document.querySelectorAll('.pro-filter-btn').forEach(b => b.classList.remove('active')); document.querySelector('.pro-filter-btn[data-filter="none"]').classList.add('active'); proPreview.style.filter = 'none'; proPreview.style.transform = 'scale(1)';
...
function generateTimelineFrames(videoEl) { const container = document.getElementById('pro-timeline-frames'); container.innerHTML = ''; const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d'); const frameCount = 5; const dur = videoEl.duration; canvas.width = 60; canvas.height = 40; let times = []; for(let i=0; i<frameCount; i++) { times.push((dur / frameCount) * i); } let i = 0; videoEl.addEventListener('seeked', function extractFrame() { if(i >= times.length) { videoEl.removeEventListener('seeked', extractFrame); videoEl.currentTime = 0; return; } ctx.drawImage(videoEl, 0, 0, canvas.width, canvas.height); const img = document.createElement('img'); img.src = canvas.toDataURL(); img.style.flex = "1"; img.style.objectFit = "cover"; img.style.height = "100%"; container.appendChild(img); i++; if(i < times.length) videoEl.currentTime = times[i]; }); if(times.length > 0) videoEl.currentTime = times[0]; }

document.getElementById('btn-pro-play').addEventListener('click', () => { const vid = document.getElementById('pro-video-source'); if (vid.paused) { vid.play(); proBgAudio.play(); document.getElementById('btn-pro-play').innerHTML = '<i class="fas fa-pause"></i>'; } else { vid.pause(); proBgAudio.pause(); document.getElementById('btn-pro-play').innerHTML = '<i class="fas fa-play"></i>'; } });

document.getElementById('pro-video-source').addEventListener('timeupdate', (e) => {
    const curr = e.target.currentTime; document.getElementById('pro-time-current').innerText = curr.toFixed(1);
    const endVal = parseFloat(document.getElementById('pro-trim-end').value) || 0; const startVal = parseFloat(document.getElementById('pro-trim-start').value) || 0;
    if(curr >= endVal && endVal > 0) { e.target.currentTime = startVal; if(proBgAudio.src) { proBgAudio.currentTime = parseFloat(document.getElementById('pro-audio-offset').value) || 0; } }
    proEditorState.overlays.forEach(obj => { const el = document.getElementById('pro-drag-txt-' + obj.id); if(el) { if(curr >= obj.start && curr <= obj.end) { el.style.display = 'block'; } else { el.style.display = 'none'; } } });
});

function updateTrim() { let s = parseFloat(document.getElementById('pro-trim-start').value); let e = parseFloat(document.getElementById('pro-trim-end').value); if(s >= e) { s = e - 0.1; document.getElementById('pro-trim-start').value = s; } document.getElementById('pro-val-start').innerText = s.toFixed(1); document.getElementById('pro-video-source').currentTime = s; proEditorState.start = s; if(proBgAudio.src) { proBgAudio.currentTime = parseFloat(document.getElementById('pro-audio-offset').value) || 0; } }
function updateTrimEnd() { let s = parseFloat(document.getElementById('pro-trim-start').value); let e = parseFloat(document.getElementById('pro-trim-end').value); if(e <= s) { e = s + 0.1; document.getElementById('pro-trim-end').value = e; } document.getElementById('pro-val-end').innerText = e.toFixed(1); document.getElementById('pro-video-source').currentTime = e - 0.1; proEditorState.end = e; }
document.getElementById('pro-trim-start').addEventListener('input', updateTrim); document.getElementById('pro-trim-start-plus').addEventListener('click', () => { document.getElementById('pro-trim-start').value = parseFloat(document.getElementById('pro-trim-start').value) + 0.1; updateTrim(); }); document.getElementById('pro-trim-start-minus').addEventListener('click', () => { document.getElementById('pro-trim-start').value = Math.max(0, parseFloat(document.getElementById('pro-trim-start').value) - 0.1); updateTrim(); });
document.getElementById('pro-trim-end').addEventListener('input', updateTrimEnd); document.getElementById('pro-trim-end-plus').addEventListener('click', () => { document.getElementById('pro-trim-end').value = Math.min(parseFloat(document.getElementById('pro-trim-end').max), parseFloat(document.getElementById('pro-trim-end').value) + 0.1); updateTrimEnd(); }); document.getElementById('pro-trim-end-minus').addEventListener('click', () => { document.getElementById('pro-trim-end').value = parseFloat(document.getElementById('pro-trim-end').value) - 0.1; updateTrimEnd(); });

document.querySelectorAll('.pro-filter-btn').forEach(btn => { btn.addEventListener('click', (e) => { document.querySelectorAll('.pro-filter-btn').forEach(b => b.classList.remove('active')); e.target.classList.add('active'); proEditorState.filter = e.target.dataset.filter; const vid = document.getElementById('pro-video-source'); if(proEditorState.filter === 'none') vid.style.filter = 'none'; if(proEditorState.filter === 'grayscale') vid.style.filter = 'grayscale(100%)'; if(proEditorState.filter === 'sepia') vid.style.filter = 'sepia(100%)'; if(proEditorState.filter === 'blur') vid.style.filter = 'blur(4px)'; if(proEditorState.filter === 'contrast') vid.style.filter = 'contrast(200%)'; if(proEditorState.filter === 'vintage') vid.style.filter = 'sepia(50%) contrast(150%)'; }); });
document.getElementById('pro-speed-ctrl').addEventListener('change', (e) => { proEditorState.speed = parseFloat(e.target.value); document.getElementById('pro-video-source').playbackRate = proEditorState.speed; proBgAudio.playbackRate = proEditorState.speed; });
document.getElementById('pro-ratio-ctrl').addEventListener('change', (e) => { proEditorState.ratio = e.target.value; const ws = document.getElementById('pro-editor-workspace'); if(proEditorState.ratio === '9:16') { ws.style.width = '196px'; } else if (proEditorState.ratio === '1:1') { ws.style.width = '350px'; } else { ws.style.width = '100%'; } });

document.getElementById('pro-audio-orig-vol').addEventListener('input', (e) => { proEditorState.origVol = parseFloat(e.target.value); document.getElementById('pro-video-source').volume = proEditorState.origVol; });
document.getElementById('pro-audio-bg-vol').addEventListener('input', (e) => { proEditorState.bgVol = parseFloat(e.target.value); proBgAudio.volume = proEditorState.bgVol; });

document.getElementById('pro-audio-upload').addEventListener('change', (e) => {
    if(e.target.files.length > 0) {
        document.getElementById('pro-trending-sounds').value = ""; // reset
        const file = e.target.files[0]; document.getElementById('pro-audio-bg-name').innerText = file.name; document.getElementById('pro-audio-controls-extended').style.display = 'block';
        const url = URL.createObjectURL(file); proBgAudio.src = url; proBgAudio.volume = proEditorState.bgVol; proBgAudio.onloadedmetadata = () => { document.getElementById('pro-audio-offset').max = proBgAudio.duration; document.getElementById('pro-audio-offset').value = 0; };
    }
});

document.getElementById('pro-trending-sounds').addEventListener('change', (e) => {
    if(e.target.value !== "") {
        document.getElementById('pro-audio-upload').value = ""; // reset
        document.getElementById('pro-audio-bg-name').innerText = e.target.options[e.target.selectedIndex].text;
        document.getElementById('pro-audio-controls-extended').style.display = 'block';
        proBgAudio.src = e.target.value; proBgAudio.volume = proEditorState.bgVol; proBgAudio.onloadedmetadata = () => { document.getElementById('pro-audio-offset').max = proBgAudio.duration; document.getElementById('pro-audio-offset').value = 0; };
    }
});
...
async function renderAndUploadImages() {
    ...
    for (let i = 0; i < imageEditorState.images.length; i++) {
        ...
        for (const obj of imageEditorState.edits[i]) {
            ...
            if(obj.type === 'sticker') { const sImg = new Image(); await new Promise(res => { sImg.onload = res; sImg.src = obj.src; }); const ratio = sImg.naturalHeight / sImg.naturalWidth; ctx.drawImage(sImg, -nativeSize/2, -(nativeSize*ratio)/2, nativeSize, nativeSize*ratio); } else if (obj.type === 'blur') { ctx.filter = 'blur(15px)'; ctx.drawImage(canvas, nativeX - nativeSize/2, nativeY - nativeSize/2, nativeSize, nativeSize, -nativeSize/2, -nativeSize/2, nativeSize, nativeSize); ctx.filter = 'none'; } else { ctx.font = `bold ${nativeSize}px ${obj.font || 'Arial, sans-serif'}`; ctx.fillStyle = obj.color; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.strokeStyle = 'rgba(0,0,0,0.8)'; ctx.lineWidth = nativeSize * 0.1; ctx.strokeText(obj.text, 0, 0); ctx.fillText(obj.text, 0, 0); }
            ctx.restore();
        }
        
        // Convert to blob and upload to Firebase Storage
        const blob = await new Promise(res => canvas.toBlob(res, 'image/jpeg', 0.9));
        const storageRef = ref(storage, `images/${currentUser.uid}_${Date.now()}_img${i}.jpg`);
...
document.getElementById('submit-upload').addEventListener('click', async() => {
    ...
    const isCloseFriendsOnly = document.getElementById('up-close-friends').checked;
    ...
    const btn = document.getElementById('submit-upload'); const status = document.getElementById('upload-status'); btn.disabled = true; status.innerText = "Lade hoch... Bitte warten!";
    
    try {
        if (isVideo) {
            // FIREBASE STORAGE UPLOAD VIDEO
            const storageRef = ref(storage, `videos/${currentUser.uid}_${Date.now()}_${files[0].name}`);
            await uploadBytesResumable(storageRef, files[0]);
            const finalUrl = await getDownloadURL(storageRef);

            // AUDIO UPLOAD (Falls eigene MP3)
            let audioUrl = null;
            const audioFiles = document.getElementById('pro-audio-upload').files;
            const trendingSound = document.getElementById('pro-trending-sounds').value;
            if (audioFiles.length > 0) {
                status.innerText = "Lade Hintergrundmusik hoch...";
                const audioRef = ref(storage, `audio/${currentUser.uid}_${Date.now()}_${audioFiles[0].name}`);
                await uploadBytesResumable(audioRef, audioFiles[0]);
                audioUrl = await getDownloadURL(audioRef);
            } else if(trendingSound !== "") {
                audioUrl = trendingSound;
            }

            // DB EINTAG (ohne Cloudinary Rendering, wir speichern die Roh-Konfiguration)
            await addDoc(collection(db, "videos"), { 
                mediaType: 'video', 
                url: finalUrl, 
                filter: proEditorState.filter,
                overlays: proEditorState.overlays,
                bgAudio: audioUrl,
                bgAudioVol: proEditorState.bgVol,
                origVol: proEditorState.origVol,
                ...
                title: titleVal, 
                description: desc, 
                likedBy: [], gifts: 0, comments: [], views: 0, timestamp: Date.now(),
                pinned: false,
            ...
            await addDoc(collection(db, "videos"), { mediaType: 'images', urls: uploadedUrls, authorUid: currentUser.uid, authorName: currentUser.displayName, authorUsername: currentUser.username, authorPic: currentUser.photoURL, authorVerified: currentUser.verified || false, title: titleVal, description: desc, likedBy: [], gifts: 0, comments: [], views: 0, timestamp: Date.now(), pinned: false, closeFriendsOnly: isCloseFriendsOnly });
        }
    ...
    } catch (e) { showCustomAlert("Upload Fehler", "Fehler! Bitte versuche es später noch einmal."); } finally { btn.disabled = false; status.innerText = ""; }
});
...
let duetStream = null;
let duetMediaRecorder = null;
let duetChunks = [];

window.openDuet = async function(videoId) {
    const video = allVideosData.find(v => v.id === videoId);
    if (!video || video.mediaType !== 'video') return showToast("Geht nur bei Videos!");
    document.getElementById('duet-original-vid').src = video.url;
    document.getElementById('duet-modal').classList.add('show');
    try {
        duetStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
        document.getElementById('duet-webcam-vid').srcObject = duetStream;
    } catch(err) { showCustomAlert("Kamera-Fehler", "Bitte erlaube den Kamera-Zugriff für das Duett."); document.getElementById('duet-modal').classList.remove('show'); }
}

window.closeDuet = function() {
    if(duetStream) { duetStream.getTracks().forEach(t => t.stop()); }
    document.getElementById('duet-original-vid').pause();
    document.getElementById('duet-modal').classList.remove('show');
}

document.getElementById('btn-start-duet').addEventListener('click', () => {
    const origVid = document.getElementById('duet-original-vid'); const camVid = document.getElementById('duet-webcam-vid');
    origVid.play(); camVid.play();
    
    const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d');
    canvas.width = 720; canvas.height = 640; // Split Screen (2x 360x640)
    
    const canvasStream = canvas.captureStream(30);
    // Audio vom Mikrofon hinzufügen
    const audioTracks = duetStream.getAudioTracks();
    if(audioTracks.length > 0) canvasStream.addTrack(audioTracks[0]);

    duetMediaRecorder = new MediaRecorder(canvasStream, { mimeType: 'video/webm' });
    duetChunks = [];
    duetMediaRecorder.ondataavailable = e => duetChunks.push(e.data);
    duetMediaRecorder.onstop = () => {
        const blob = new Blob(duetChunks, { 'type': 'video/webm' });
        const file = new File([blob], "duet.webm", { type: 'video/webm' });
        
        // Simuliere einen Upload im Upload-Modal
        const dt = new DataTransfer(); dt.items.add(file);
        document.getElementById('up-file').files = dt.files;
        
        // Trigger Change Event
        const event = new Event('change', { bubbles: true });
        document.getElementById('up-file').dispatchEvent(event);
        
        closeDuet();
        document.getElementById('upload-modal').classList.add('show');
        showToast("Duett fertig! Du kannst es jetzt posten.");
    };

    duetMediaRecorder.start();
    document.getElementById('btn-start-duet').style.display = 'none';
    document.getElementById('btn-stop-duet').style.display = 'block';

    function drawLoop() {
        if(duetMediaRecorder.state === "recording") {
            ctx.drawImage(origVid, 0, 0, 360, 640);
            ctx.drawImage(camVid, 360, 0, 360, 640);
...
document.getElementById('btn-stop-duet').addEventListener('click', () => {
    if(duetMediaRecorder && duetMediaRecorder.state === "recording") duetMediaRecorder.stop();
    document.getElementById('duet-original-vid').pause();
    document.getElementById('btn-start-duet').style.display = 'block';
    document.getElementById('btn-stop-duet').style.display = 'none';
});
